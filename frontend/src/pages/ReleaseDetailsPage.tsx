...
export function ReleaseDetailsPage() {
  ...
  const [release, setRelease] = useState<Release | null>(null); const [info, setInfo] = useState(""); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [saving, setSaving] = useState(false);
  ...
  const toggle = async (name: string) => {
    if (!release) return; const previous = release; const steps = { ...release.steps, [name]: !release.steps[name] }; const completed_steps = Object.values(steps).filter(Boolean).length; const optimistic: Release = { ...release, steps, completed_steps, status: completed_steps === 0 ? "planned" : completed_steps === release.total_steps ? "done" : "ongoing" }; setRelease(optimistic);
    try { setRelease(await releaseApi.updateSteps(release.id, steps)); notify("Checklist updated"); } catch { setRelease(previous); notify("Update failed — your change was restored", "error"); }
  };
  ...
  return <><Toasts messages={messages} /><div className="mx-auto max-w-5xl px-4 py-8 sm:px-6"><Link to="/" className="mb-7 inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-brand"><ArrowLeft size={18} />All releases</Link>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[1.25fr_.75fr]"><section className="panel"><p className="eyebrow">Release checklist</p><h2 className="section-title">Readiness steps</h2><div className="mt-5 divide-y divide-slate-100">{STEP_NAMES.map((name, index) => <label key={name} className="flex cursor-pointer items-center gap-4 py-4"><input type="checkbox" checked={release.steps[name]} onChange={() => toggle(name)} className="h-5 w-5 rounded border-slate-300 accent-brand" /><span className={`flex-1 font-semibold ${release.steps[name] ? "text-slate-400 line-through" : ""}`}>{name}</span><span className="text-xs font-bold text-slate-400">{String(index + 1).padStart(2, "0")}</span></label>)}</div></section>
